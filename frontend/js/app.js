...
/* ============================================================
   TAB 3 — TEKNİK SEO
============================================================ */
document.getElementById('t3-audit-btn').addEventListener('click', runPageSpeedAudit);

// Gerçek Google PageSpeed Insights (Lighthouse) API entegrasyonu 
//  Google Cloud Console'dan aldığım PageSpeed API key'i
const PAGESPEED_API_KEY = 'AIzaSyAUiS933QNJ0XA8sRLP8YvfZtrXde0khLM';
...
async function runPageSpeedAudit(){
  ...
  const endpoint = 'https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url='
    + encodeURIComponent(url) + '&strategy=mobile&category=performance&category=seo&category=accessibility&category=best-practices&key=' + PAGESPEED_API_KEY;
  ...
  try{
    ...
    if(!audits || !categories) throw new Error('Lighthouse verisi bulunamadı.');

    // 4 Ana Kategori Skorunu Çekme (0-100)
    ...
    // Çemberleri Güncelleyen Minik Yardımcı Fonksiyon
    ...
    setSmallGauge('t3-perf', perfScore);
    setSmallGauge('t3-seo', seoScore);
    setSmallGauge('t3-a11y', a11yScore);
    setSmallGauge('t3-bp', bpScore);

    // Detaylı Hız Metriklerini (Web Vitals) Çekme
    ...
    const lcpSec = lcpAudit?.numericValue ? lcpAudit.numericValue / 1000 : null;
    const fcpSec = fcpAudit?.numericValue ? fcpAudit.numericValue / 1000 : null;
    const clsVal = clsAudit?.numericValue ?? null;
    const ttfbSec = ttfbAudit?.numericValue ? ttfbAudit.numericValue / 1000 : null;
    const inpMs = inpAudit?.numericValue ?? null;

    renderMeter('t3-lcp', lcpAudit?.displayValue || '—', lcpSec !== null ? lcpStatus(lcpSec) : 'mid', lcpSec !== null ? lcpPercent(lcpSec) : 50);
    renderMeter('t3-fcp', fcpAudit?.displayValue || '—', fcpSec !== null ? (fcpSec <= 1.8 ? 'good' : fcpSec <= 3.0 ? 'mid' : 'bad') : 'mid', fcpSec !== null ? lcpPercent(fcpSec) : 50);
    renderMeter('t3-cls', clsAudit?.displayValue || '—', clsVal !== null ? clsStatus(clsVal) : 'mid', clsVal !== null ? clsPercent(clsVal) : 50);
    renderMeter('t3-ttfb', ttfbAudit?.displayValue || '—', ttfbSec !== null ? (ttfbSec <= 0.8 ? 'good' : ttfbSec <= 1.8 ? 'mid' : 'bad') : 'mid', ttfbSec !== null ? lcpPercent(ttfbSec) : 50);
    renderMeter('t3-inp', inpAudit?.displayValue || '—', inpMs !== null ? (inpMs <= 200 ? 'good' : inpMs <= 500 ? 'mid' : 'bad') : 'mid', inpMs !== null ? Math.max(0, 100 - (inpMs/10)) : 50);

    document.getElementById('t3-audit-url').textContent = url;
    document.getElementById('t3-output-card').classList.remove('hidden');
    
    // Teknik SEO genel skoru için 4 kategorinin ortalamasını hesapla
    const averageTechScore = Math.round((perfScore + seoScore + a11yScore + bpScore) / 4);
    state.technicalScore = averageTechScore;
    
    // Hızlı Teknik Denetleyici (Robots, Sitemap, SSL)
    document.getElementById('t3-quick-audit-card').classList.remove('hidden');
    const isHttps = url.startsWith('https://');
    document.getElementById('t3-qa-ssl').innerHTML = isHttps 
      ? '<span class="tag tag--success" style="font-weight:500;">Aktif (Güvenli)</span>'
      : '<span class="tag tag--danger" style="font-weight:500;">Pasif (Güvensiz)</span>';
    
    document.getElementById('t3-qa-robots').innerHTML = '<span class="spinner" style="width:12px;height:12px;border-width:2px;display:inline-block;vertical-align:middle;border-top-color:#fff;"></span>';
    document.getElementById('t3-qa-sitemap').innerHTML = '<span class="spinner" style="width:12px;height:12px;border-width:2px;display:inline-block;vertical-align:middle;border-top-color:#fff;"></span>';

    try {
      // url input is guaranteed to be somewhat valid because of urlInput.value.trim(). 
      // Ensure we have a valid URL object
      let parsedUrl = url.startsWith('http') ? new URL(url) : new URL('https://' + url);
      const robotsUrl = new URL('/robots.txt', parsedUrl.origin).href;
      const sitemapUrl = new URL('/sitemap.xml', parsedUrl.origin).href;
      
      fetch('http://localhost:3000/api/ping?url=' + encodeURIComponent(robotsUrl))
        .then(r => r.json())
        .then(data => {
          document.getElementById('t3-qa-robots').innerHTML = data.ok 
            ? '<span class="tag tag--success" style="font-weight:500;">Bulundu (200 OK)</span>'
            : `<span class="tag tag--danger" style="font-weight:500;">Bulunamadı (${data.status})</span>`;
        }).catch(e => {
          document.getElementById('t3-qa-robots').innerHTML = '<span class="tag tag--warn" style="font-weight:500;">Test Edilemedi (Ağ/Hata)</span>';
        });

      fetch('http://localhost:3000/api/ping?url=' + encodeURIComponent(sitemapUrl))
        .then(r => r.json())
        .then(data => {
          document.getElementById('t3-qa-sitemap').innerHTML = data.ok 
            ? '<span class="tag tag--success" style="font-weight:500;">Bulundu (200 OK)</span>'
            : `<span class="tag tag--danger" style="font-weight:500;">Bulunamadı (${data.status})</span>`;
        }).catch(e => {
          document.getElementById('t3-qa-sitemap').innerHTML = '<span class="tag tag--warn" style="font-weight:500;">Test Edilemedi (Ağ/Hata)</span>';
        });
    } catch(err) {
      document.getElementById('t3-qa-robots').innerHTML = '<span class="tag tag--danger" style="font-weight:500;">Geçersiz URL</span>';
      document.getElementById('t3-qa-sitemap').innerHTML = '<span class="tag tag--danger" style="font-weight:500;">Geçersiz URL</span>';
    }
    ...
    btn.disabled = false; label.textContent = 'Canlı Denetim Yap';
  }
}

// 429 (rate limit) durumunda birkaç kez tekrar dener
async function fetchWithRetry(endpoint, retries = 3, delay = 6000){
  ...
  // Son denemede de 429 aldıysak, Google'ın gerçek mesajını çıkar
  const errBody = await lastResponse.json().catch(() => null);
  const detail = errBody && errBody.error && errBody.error.message
    ? errBody.error.message
    : 'bilinmeyen sebep';
  throw new Error('PageSpeed API yanıt kodu: 429 (' + detail + ')');
}
    ...
    rows.forEach((tr, i) => {
      ...
      if(tr.dataset.id){
        supabaseClient.from('backlinks')
          .update({ quality_label: labelVal, quality_score: scoreVal })
          .eq('id', tr.dataset.id)
          .then(({ error }) => {
            if(error) console.warn('[Supabase] backlink kalite bilgisi kaydedilemedi:', error.message);
          });
      }
    });
